...
class HttpHelper {
  ...
  Future<Either<Failure, Success>> handlePutRequest(
    ...
    Future<Either<Failure, Success>> handleDeleteRequest(String api, Map<String, dynamic> data, { showSuccessToast = true, showLoader = true}) async {
    try {
      showL = showLoader;
      if (showLoader) {
        _showLoader();
      }
      final res = await _dio.delete('${AppConfig.shared.baseUrl}/$api', data: data);
      if (showSuccessToast) {
        _showMessage((res.data?['message'] ?? res.data.toString()).toString());
      }
      return Right(Success<Map<String, dynamic>?>(res.data));
